...
public class HeroController
{
    ...
    /// <summary>
    /// 跳过战斗，直接设置玩家和敌人的当前血量和怒气为跳过时的状态,然后结束战斗
    /// </summary>
    /// <param name="args"></param>
    public void SkipBattle(SkipEvent args)
    {
        //直接修改玩家和敌人的当前血量和怒气为跳过时的状态
        for (int i = 0; i < playerList.Count; i++)
        {
            playerList[i].data.nowHp = args.playerNowHp[i];
            playerList[i].data.currentRage = args.playerNowRage[i];
        }
        for (int i = 0; i < enemyList.Count; i++)
        {
            enemyList[i].data.nowHp = args.enemyNowHp[i];
            enemyList[i].data.currentRage = args.enemyNowRage[i];
        }
    }
